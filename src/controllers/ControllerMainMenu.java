...
/**
 * Materia: Laboratorio I Sección: 1 Integrantes:
 *
...
public class ControllerMainMenu implements ActionListener, MouseListener {
     ...
     * @param nameUser Nombre y Apellido del usuario que ha ingresado al
     * sistema.
    ...
    public void actionPerformed(java.awt.event.ActionEvent evt) {
        ...
        // Volver para la sección anterior
            ...
            if (mainPage.cmbCandySelection.getSelectedIndex() == 0) // Se muestra un mensaje de que la venta fue generada con éxito.
            {
                popup = new PopupMessage(mainPage, true,
                        1, "Seleccione alguna golosina.");
            } // Si no se ha indicado una cantidad de golosina a agregar.
            else if ((int) mainPage.spnCantCandySell.getValue() == 0) // Se muestra un mensaje de que la venta fue generada con éxito.
            {
                popup = new PopupMessage(mainPage, true,
            ...
            /**
             * Se verifica que estén todos los datos. Se verifica: -
             * txtIdClientCandySell: Que no esté vacío ni valor predeterminado.
             * - lblSucursalnameCandySell: Que no esté vacío ni valor
             * predeterminado. - cmbCandySeller: Que no esté en el valor
             * predeterminado. - getRowCount(): Que se haya ingresado al menos
             * una golosina.
             */
            if ((!"".equals(mainPage.txtIdClientCandySell.getText())
                ...
                if (option == javax.swing.JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        if (codexInv.size() > 0) {
                            codexNumber = Integer.valueOf(codexInv.get(codexInv.size() - 1).substring(4)) + 1;
                        }
        ...
        // Volver al paso anterior.
             ...
             * - lblSucursalnameCinemaSell: Que no esté vacío ni valor
             * predeterminado. - cmbCinemaSeller: Que no esté en el valor
             * predeterminado. - getRowCount(): Que se haya ingresado al menos
             * una golosina.
             */
                ...
                if (option == javax.swing.JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < dtm.getRowCount(); i++) {
                            names.add(dtm.getValueAt(i, 0).toString());
                            seats.add(1);
                            String amountString = dtm.getValueAt(i, 4).toString();
                            amounts.add(Double.valueOf(amountString));
                        }

                        ArrayList<String> enterpriseData = loadEnterprise();
                        ArrayList<String> employeeData = loadOnlyOneEmployee("Esteban", "Dido", 2);
                        ArrayList<String> clientData = loadOnlyOneClient("26772857");
                        ArrayList<String> idTickets = new ArrayList<>();

                        idTickets.add(String.valueOf((int) java.lang.Math.random() * 5000) + 1);

                        String idTicket = "0";

                        ArrayList<String> codexTicket = loadCodexOfTickets(1);

                        if (codexTicket.size() > 0) {
                            idTicket = codexTicket.get(codexTicket.size() - 1).substring(4);
                        }
                        ...
                        String seller = mainPage.cmbCinemaSeller.getSelectedItem().toString();
                        String[] sellerName = seller.split(" ");
                        ...
                        suportList = loadOnlyOneClient(mainPage.txtIdClientCinemaSell.getText());
        ...
        /*else if(evt.getSource() == mainPage.btnUserDecision)
        {
            ...
            ControllerUserManagement contUserManagement = 
                    new ControllerUserManagement (rolUser, nameUser, nameBranch);*/ else if (evt.getSource() == mainPage.btnUserDecision) {
     ...
     *
     * @param idMovie Código de la película de la que se van a buscar sus
     * funciones.
     */
    ...
    /**
     * Método que contiene le procedimiento a seguir para el tercer paso en la
    ...
    /**
     * Método para mostrar todos los tickets y su información al respecto que se
     * van a comprar.
     */
